...
class _HomeScreenState extends State<HomeScreen> {
  ...
  final List<Map<String, dynamic>> _manualLocations = [
    ...
    {
      'name': 'Office Magetan',
      'location': LatLng(-7.6562, 111.3280),
    },
  ];
  ...
  Future<void> checkPresenceStatus() async {
    setState(() {
    ...
    try {
      final response = await http.get(
        Uri.parse('http://192.168.100.22/api/presencecheck?nama=$_userName'),
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              ...
              ElevatedButton(
                onPressed: _hasCheckedIn ? null : () => checkPresenceStatus(),
                style: _hasCheckedIn 
                  ? ElevatedButton.styleFrom(backgroundColor: Colors.grey)
                  : ElevatedButton.styleFrom(backgroundColor: Colors.blue),
                child: _isLoadingPresence
                  ? const CircularProgressIndicator(color: Colors.white)
                  : Text(_hasCheckedIn ? 'Already Present' : 'Present'),
              ),
            ],
          ),
        ),
  ...
  try {
    final response = await http.post(
      Uri.parse('http://192.168.100.22:5000/api/getlocation'),
      headers: {
